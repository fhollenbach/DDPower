...
firm_simulation_scrambled <- function(i){
    ...
    AutoCor = simulation_para$AutoCor[i]
    ...
    sim_firms <- tibble(
      ...
      first_treated = c(rep(0, NUnits - (group_sizes*3)), rep(11, group_sizes),  rep(16, group_sizes),  rep(21, group_sizes))) %>%
    ...
    sim_year_fe <- tibble(
      time = 1:25,
      year_fe = sample(year_fes, 25, replace = TRUE)
    )

    dat <- crossing(sim_firms, sim_year_fe) %>% 
        mutate(resid = sample(mod$residuals, NUnits*25, replace = TRUE),
           roa = firm_fe + year_fe + resid) %>%
           arrange(id, time) %>% 
           group_by(id) %>% 
           mutate(lag.roa = lag(roa, order_by = time),
                  roa = if_else(is.na(lag.roa) == FALSE, roa + AutoCor*lag.roa, roa + AutoCor*sample(mod$residuals, 1))) %>%
           ungroup() %>% 
        mutate(sd_roa = sd(roa),### sd
               pre_treat_roa = mean(roa[treated == 1 & time <= first_treated])) %>%  ### pre treatment mean roa for treated units
           ...
           mutate(post_period = if_else(treated == 1 & time >= first_treated, 1, 0), ## indicator for post treatment for treated units
                  ...
                  group_specific_effect = case_when(first_treated == 11 ~ (EffectSize*sd_roa / 3),
                                             first_treated == 16 ~ (EffectSize*sd_roa),
                                             first_treated == 21 ~ (EffectSize*sd_roa * 3),
    ...
    avg_att <- dat %>%
      ...
      pull(ate) %>%
      mean()
    ###### event time true atts
    att_time <- dat %>%
      filter(post_period == 1 & time_since_treatment %in% 0:4) %>% 
      group_by(time_since_treatment) %>%
      summarize(att = mean(ate))
   ...
   # callaway and sant'anna
    ...
    results <- bind_rows(twfe, wooldridge, csa, bjs, gardner, sab) %>% 
      mutate(true_att = avg_att,
             pretreatment_roa = unique(dat$pre_treat_roa),
             sd_roa = unique(dat$sd_roa))
    results <- results %>% 
...
firm_simulation_panel <- function(i){
  ...
  #### draw firms for each group 
  firms <- tibble(gvkey = sample(unique(comp$gvkey), NUnits, replace = FALSE),
  ...
  if(simulation_para$DV[i] == "roa"){
    ...
    comp$dv <- comp$log_rev
  } ### select dependent var)
  ### draw unique firms based on number of units
  dat <- comp %>% 
    select(dv, gvkey, time) %>%
    filter(gvkey %in% firms$gvkey & time <= 20) %>%
    left_join(firms, by = "gvkey") %>% 
    mutate(treated = if_else(first_treated == 0, 0, 1),
           first_treated_sa = if_else(first_treated == 0, 1000, first_treated)) %>% ### special variable for S&A
    arrange(gvkey, time) %>% 
    ungroup() %>% 
    mutate(sd_dv = sd(dv),### sd
           pre_treat_dv = mean(dv[treated == 1 & time <= first_treated])) %>%  ### pre treatment mean dv for treated units
    ### next useful variable to be used later
    mutate(post_period = if_else(treated == 1 & time >= first_treated, 1, 0), ## indicator for post treatment for treated units
           time_since_treatment = if_else(first_treated != 0, time - first_treated, 0), #### relative time indicator
  ...
  dat <- dat %>%
    mutate(time = as.integer(time),
           ...
           time_to_treatment = case_when(first_treated == 0 ~ -1000,
                                         TRUE ~ time - first_treated))
  if(i %in% iterations_save){
    write_csv(dat[, c("y", "gvkey", "time", "post_period")], here("SimData_firms", paste0("Data_firms_",i,".csv")))  
  }
  
  dat <- na.omit(dat)
  ### twfe
  twfe <- feols(y ~ i(post_period, 0) | gvkey + time, data = dat) %>%
    tidy(conf.int = TRUE) %>%
    mutate(t = -100, 
           method = "TWFE") %>%
    select(t, estimate, std.error, method)
  ...
  wooldridge_est <- etwfe(
    ...
    gref = 0, # group variable (with bespoke ref. level)
    data = dat,                 # dataset
    vcov = ~gvkey)            # vcov adjustment (here: clustered) 
  ...
  # 
  dat$time_impute <- as.numeric(dat$time)
  dat$id_impute <- as.numeric(dat$gvkey)
  dat$dv_impute <- as.numeric(dat$y)
  ...
  # Sun and Abraham
  sab_est <- feols(y ~ sunab(first_treated_sa, time) | gvkey + time,
                   cluster ~ gvkey,
                   data = dat) 
  sab <- sab_est %>%
    summary(agg = "att") %>%
  ...
  results <- bind_rows(twfe, wooldridge, csa, bjs, gardner, sab) %>% 
    ...
    bind_cols(slice(simulation_para[i,], rep(1, each = 6)))
  write_parquet(results, here("SimResults_firms", paste0("results_firms",i,".parquet")))
  return(NULL)
}
